<?php
	...
	*/
	...
	function getMethod()
	{
		...
		$query = "SELECT * FROM `urgencias_organos` u, `organos_donables` o WHERE u.id_organo = o.id_organo";
		...
		if($data->EOF){
			...
			while($data && !$data->EOF)
			{
				$urgenca[$i]['rut'] = $data->fields[4];
				$urgenca[$i]['nombre_paciente'] = $data->fields[5];
				$urgenca[$i]['apellido_pat'] = $data->fields[6];
				$urgenca[$i]['apellido_mat'] = $data->fields[7];
				$urgenca[$i]['grado_urgencia'] = $data->fields[9];
				$urgenca[$i]['organo'] = $data->fields[13];
				$i++;
				$data->MoveNext();
			}
