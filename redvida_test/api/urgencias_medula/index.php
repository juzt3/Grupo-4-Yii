<?php
	...
	*/
	...
	function getMethod()
	{
		...
		$query = "SELECT * FROM `urgencias_medula`";
		...
		if($data->EOF){
			...
			while($data && !$data->EOF)
			{
				$urgenca[$i]['rut'] = $data->fields[3];
				$urgenca[$i]['nombre_paciente'] = $data->fields[4];
				$urgenca[$i]['apellido_pat'] = $data->fields[5];
				$urgenca[$i]['apellido_mat'] = $data->fields[6];
				$urgenca[$i]['grado_urgencia'] = $data->fields[8];
				$urgenca[$i]['tipo_transplante'] = $data->fields[9];
				$i++;
				$data->MoveNext();
			}
